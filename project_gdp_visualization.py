...
def reconcile_countries_by_name(plot_countries, gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    ...
    Set=set()
    Tup1=(dict_1,Set)
    for wo1 in plot_countries.keys():
        for values in gdp_countries.values():
            if plot_countries[wo1]==values['Country Name']:
               for year in range(1960,2016):
                   if values[str(year)]!="":
                      dict_1[wo1]=values
                      
    for wo2 in plot_countries.keys():

        if wo2 not in dict_1:
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
   ...
   """
    ...
    csvfile=open(gdpinfo['gdpfile'],'rt')
    readers=csv.DictReader(csvfile,delimiter=gdpinfo["separator"],quotechar=gdpinfo["quote"])
    for cow in readers:
